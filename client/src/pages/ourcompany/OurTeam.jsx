...
export default function OurTeam() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Marquee animation setup — only once team data is loaded
      let handleMouseEnter;
      let handleMouseLeave;

      if (marqueeInnerRef.current && marqueeRef.current && teamData.length > 0) {
        ...
        handleMouseEnter = () => marqueeAnimation.current?.pause();
        handleMouseLeave = () => marqueeAnimation.current?.play();
      ...
      return () => {
        if (marqueeRef.current && handleMouseEnter) {
                  ...
                  >
                    <div className="bg-white rounded-2xl shadow p-0 hover:shadow-xl transition-all duration-300 min-h-[220px] md:min-h-[280px] relative overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex flex-col justify-center items-center p-4 sm:p-6 transition-all duration-500 ease-in-out group-hover:opacity-0 group-hover:scale-95">
                        <img
                          src={member.image ? `${SERVER_URL}${member.image}` : '/assets/ourteam/placeholder.jpg'}
                          alt={member.name}
